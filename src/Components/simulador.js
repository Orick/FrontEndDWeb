...
class Simulador extends Component {
    ...
    calcularDamage(champ1,champ2,iditem11,iditem12,iditem13,iditem14,iditem15,iditem16,iditem21,iditem22,iditem23,iditem24,iditem25,iditem26){
        fetch('http://' + localhost + '/simulador/attack/' + champ1 + '/' + iditem11 + '/' + iditem12 + '/' + iditem13 + '/' + iditem14 + '/' + iditem15 + '/' + iditem16)
        .then(response => response.json())
        .then(attack => {
            fetch('http://' + localhost + '/simulador/recibe/' + champ2 + '/' + iditem21 + '/' + iditem22 + '/' + iditem23 + '/' + iditem24 + '/' + iditem25 + '/' + iditem26)
            .then(response => response.json())
            .then(recibe => {
    ...
    updateBuilds(){
        let user = firebase.auth().currentUser;
        if (user) {
            fetch('http://' + localhost + '/builds/' + user.uid)
            .then(response => response.json())
            .then(result => {
                this.setState({
    ...
    selectBuild(build){
        let user = firebase.auth().currentUser;
        if (user) {
            fetch('http://' + localhost + '/builds/obtener/' + user.uid + '/' + build)
            .then(response => response.json())
            .then(result => {
                this.setState({
    ...
    componentDidMount(){
        fetch('http://' + localhost + '/items/data/basic')
        .then(response => response.json())
        .then(result => {
            this.setState({
        ...
        fetch('http://' + localhost + '/champions/data/basic')
        .then(response => response.json())
        .then(result => {
            this.setState({
